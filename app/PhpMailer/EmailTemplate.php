...
class EmailTemplate extends SMTPCon
{
    ...
    public function OTPVerificationTemplate($user_email, $otp_pin)
    {
        if ($this->is_connected_to_smtp) {
            $args = [
                "address" => $user_email,
                "contact_subject" => "Riders Buddy OTP",
                "mail_content" => "<img src='https://i.imgur.com/yNYMmIZ_d.png?maxwidth=760&fidelity=grand' width='160' alt='Riders Buddy'/><br/><hr/><br/><h2>Your OTP code is </h2><p>{$otp_pin}</p>",
            ];
    ...
    public function OrderUpdateTemplate($user_email, $contact_subject, $msg_body, $order_detail, $order)
    {
        ...
        <hr/>";
            ...
            $msg_body = "<img src='https://i.imgur.com/yNYMmIZ_d.png?maxwidth=760&fidelity=grand' width='160' alt='Riders-Buddy'/><br/><hr/><br/>" . $msg_body;
            $msg_body .= '<a href="http://localhost:3000/orders/' . $order['order_id'] . '" style="padding: 16px; color: #fff; text-decoration: none; font-weight: bold; border-radius: 32px;background: #1AA3E9;display: inline-block;margin-top: 16px;">View Order</a>';
            $args = [
                "address" => $user_email,
